...
public class WaitQueueWorker extends Thread {
    ...
    public void run() {
        while (true) {
            ...
            synchronized (crawlerInstance) {
                if(crawlerInstance.getCrawlerDataStructure().getJobsScheduled().size() == 0 && crawlerInstance.getCrawlerDataStructure().getQueuesSize() == 0) {
                    break;
                }
                if(!crawlerInstance.getCrawlerDataStructure().canIIndexNewDocument()) {
                    crawlerInstance.getCrawlerDataStructure().getReadyQueue().clear();
                    crawlerInstance.getCrawlerDataStructure().getWaitQueue().clear();
                    break;
                }
            }
    ...
    public void endTopology()
    {
        this.crawlerThreadState = CrawlWorker.CrawlerThreadState.TERMINATE;
        crawlerInstance.tearDownTopology(false,false);
    }
